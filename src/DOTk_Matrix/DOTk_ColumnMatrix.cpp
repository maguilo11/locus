...
DOTk_ColumnMatrix<ScalarType>::DOTk_ColumnMatrix(const dotk::Vector<ScalarType> & aColumn, size_t aNumColumns) :
        dotk::matrix<ScalarType>(),
        m_Size(aColumn.size() * aNumColumns),
        m_NumRows(aColumn.size()),
        m_NumColumns(aNumColumns),
        m_MatrixData(new std::shared_ptr<dotk::Vector<ScalarType>>[aNumColumns])
{
    this->initialize(aColumn);
}
...
template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::copy(const size_t & aIndex,
                                         const dotk::Vector<ScalarType> & aInput,
                                         bool column_major_copy_)
{
    if(column_major_copy_ == false)
    {
        assert(aInput.size() == this->ncols());

        size_t column;

        for(column = 0; column < aInput.size(); ++column)
        {
            m_MatrixData[column]->operator[](aIndex) = aInput.operator[](column);
        }
    }
    else
    {
        assert(aInput.size() == this->nrows());
        m_MatrixData[aIndex]->update(1., aInput, 0.);
    }
}

template<typename ScalarType>
ScalarType DOTk_ColumnMatrix<ScalarType>::norm(const size_t & aIndex, bool column_major_norm_) const
{
    ScalarType value = 0.;
    ...
    if(column_major_norm_ == false)
    {
        ...
        {
            value += m_MatrixData[column]->operator[](aIndex) * m_MatrixData[column]->operator[](aIndex);
        }
    ...
    {
        value = m_MatrixData[aIndex]->norm();
    }
...
template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::scale(const size_t & aIndex, const ScalarType & alpha_, bool column_major_scale_)
{
    ScalarType value = 0.;
    ...
    if(column_major_scale_ == false)
    {
        ...
        {
            value = m_MatrixData[column]->operator[](aIndex) * alpha_;
            m_MatrixData[column]->operator[](aIndex) = value;
        }
    }
    else
    {
        m_MatrixData[aIndex]->scale(alpha_);
    }
}

template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::axpy(const size_t & aIndex,
                                         ...
                                         bool column_major_axpy_)
{
    ...
    if(column_major_axpy_ == false)
    {
        ...
        for(column = 0; column < input_.size(); ++ column)
        {
            ...
            value = m_MatrixData[column]->operator[](aIndex) + scaled_value;
            m_MatrixData[column]->operator[](aIndex) = value;
        }
    }
    else
    {
        ...
        m_MatrixData[aIndex]->update(alpha_, input_, 1.);
    }
}

template<typename ScalarType>
ScalarType DOTk_ColumnMatrix<ScalarType>::dot(const size_t & aIndex,
                                  const dotk::Vector<ScalarType> & input_,
                                  bool column_major_dot_) const
{
    ...
    if(column_major_dot_ == false)
    {
        ...
        for(column = 0; column < input_.size(); ++ column)
        {
            result += m_MatrixData[column]->operator[](aIndex) * input_.operator[](column);
        }
    }
    else
    {
        assert(this->nrows() == input_.size());
        result = m_MatrixData[aIndex]->dot(input_);
    }
...
template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::fill(const ScalarType & aValue)
{
    for(size_t column = 0; column < this->ncols(); ++ column)
    {
        m_MatrixData[column]->fill(aValue);
    }
}
...
template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::set(const size_t & aRowIndex, const size_t & aColumnIndex, ScalarType aValue)
{
    assert(aRowIndex <= this->nrows() - 1u);
    assert(aColumnIndex <= this->ncols() - 1u);
    m_MatrixData[aColumnIndex]->operator[](aRowIndex) = aValue;
}
...
template<typename ScalarType>
std::shared_ptr<dotk::Vector<ScalarType> > & DOTk_ColumnMatrix<ScalarType>::basis(const size_t & aIndex)
{
    return (m_MatrixData[aIndex]);
}

template<typename ScalarType>
ScalarType & DOTk_ColumnMatrix<ScalarType>::operator ()(const size_t & aRowIndex, const size_t & aColumnIndex)
{
    assert(aRowIndex <= this->nrows() - 1u);
    assert(aColumnIndex <= this->ncols() - 1u);
    return (m_MatrixData[aColumnIndex]->operator[](aRowIndex));
}

template<typename ScalarType>
const ScalarType & DOTk_ColumnMatrix<ScalarType>::operator ()(const size_t & aRowIndex, const size_t & aColumnIndex) const
{
    assert(aRowIndex <= this->nrows() - 1u);
    assert(aColumnIndex <= this->ncols() - 1u);
    return (m_MatrixData[aColumnIndex]->operator[](aRowIndex));
...
template<typename ScalarType>
std::shared_ptr<dotk::matrix<ScalarType> > DOTk_ColumnMatrix<ScalarType>::clone() const
{

    size_t basis_dim = this->basisDimension();
    std::shared_ptr<dotk::Vector<ScalarType> > & basis_vector = m_MatrixData[0];
    std::shared_ptr<dotk::serial::DOTk_ColumnMatrix<ScalarType> >
        matrix(std::make_shared<dotk::serial::DOTk_ColumnMatrix<ScalarType>>(*basis_vector, basis_dim));
...
template<typename ScalarType>
void DOTk_ColumnMatrix<ScalarType>::initialize(const dotk::Vector<ScalarType> & aColumn)
{
    for(size_t index = 0; index < this->ncols(); ++ index)
    {
        m_MatrixData[index] = aColumn.clone();
    }
}
