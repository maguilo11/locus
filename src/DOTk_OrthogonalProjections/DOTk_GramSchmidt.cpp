...
DOTk_GramSchmidt::DOTk_GramSchmidt(const std::shared_ptr<dotk::DOTk_Primal> & primal_, size_t krylov_subspace_dim_) :
...
void DOTk_GramSchmidt::setOrthogonalVector(size_t index_, const std::shared_ptr<dotk::Vector<Real> > & vec_)
{
    m_OrthogonalBasis[index_]->update(1., *vec_, 0.);
}

const std::shared_ptr<dotk::Vector<Real> > & DOTk_GramSchmidt::getOrthogonalVector(size_t index_) const
{
...
void DOTk_GramSchmidt::setLinearOperatorTimesOrthoVector(size_t index_, const std::shared_ptr<dotk::Vector<Real> > & vec_)
{
    m_LinearOperatorTimesOrthoVector[index_]->update(1., *vec_, 0.);
}

const std::shared_ptr<dotk::Vector<Real> > & DOTk_GramSchmidt::getLinearOperatorTimesOrthoVector(size_t index_) const
{
    return (m_LinearOperatorTimesOrthoVector[index_]);
}

void DOTk_GramSchmidt::initialize(const std::shared_ptr<dotk::DOTk_Primal> & primal_)
{
