/*
 * DOTk_ColumnMatrix.hpp
 *
 *  Created on: Jul 1, 2015
 *      Author: Miguel A. Aguilo Valentin
 */

#ifndef DOTK_COLUMNMATRIX_HPP_
#define DOTK_COLUMNMATRIX_HPP_
...
template<class Type>
class DOTk_ColumnMatrix: public dotk::matrix<Type>
{
public:
    DOTk_ColumnMatrix(const dotk::vector<Type> & column_, size_t storage_size_);
    ...
    virtual Type norm(const size_t & index_, bool column_major_dot_product_ = true) const;
    // Scales a vector by a real constant. Index indicates row/column index.
    virtual void scale(const size_t & index_, const Type & alpha_, bool column_major_scale_ = true);
...
private:
    void clear();
    void initialize(const dotk::vector<Type> & column_);
...
private:
    DOTk_ColumnMatrix(const dotk::serial::DOTk_ColumnMatrix<Type> &);
    dotk::serial::DOTk_ColumnMatrix<Type> & operator=(const dotk::serial::DOTk_ColumnMatrix<Type> & rhs_);
};
