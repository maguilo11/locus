...
{
public:
    vector(int global_dim_, int num_threads_, Type value_ = 0.);
    vector(MPI_Comm comm_, int global_dim_, int num_threads_, Type value_ = 0.);
    virtual ~vector();
...
private:
    vector(const dotk::mpix::vector<Type> &);
    dotk::mpix::vector<Type> & operator=(const dotk::mpix::vector<Type> & rhs_);
};
