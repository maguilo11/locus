...
{
public:
    array(int global_dim_, int num_threads_, Type value_ = 0.);
    array(MPI_Comm comm_, int global_dim_, int num_threads_, Type value_ = 0.);
    virtual ~array();
...
private:
    array(const dotk::mpix::array<Type> & rhs_);
    dotk::mpix::array<Type> & operator=(const dotk::mpix::array<Type> & rhs_);
};
