...
{
public:
    vector(std::vector<Type> & data_, int num_threads_ = 1);
    vector(int dim_, int num_threads_ = 1, Type value_ = 0.);
    virtual ~vector();
...
private:
    vector(const dotk::omp::vector<Type> &);
    dotk::omp::vector<Type> & operator=(const dotk::omp::vector<Type> & rhs_);
};
