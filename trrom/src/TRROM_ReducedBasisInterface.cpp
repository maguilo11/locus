...
void ReducedBasisInterface::updateOrthonormalBases()
{
    ...
    int num_state_basis_vectors = m_StateBasis->numCols();
    ...
    if(m_SpectralDecompositionMng->areDualSnapshotsCollected() == true)
    {
        ...
        int num_dual_basis_vectors = m_DualBasis->numCols();
...
void ReducedBasisInterface::updateReducedStateRightHandSide(const std::tr1::shared_ptr<trrom::Vector<double> > & high_fidelity_rhs_)
{
...
void ReducedBasisInterface::updateLeftHandSideDeimDataStructures()
{
    ...
    int num_columns = m_LeftHandSideBasis->numCols();
...
void ReducedBasisInterface::updateReducedLeftHandSideEnsembles()
{
    ...
    int num_state_degrees_of_freedom = m_StateBasis->numRows();
...
void ReducedBasisInterface::solveLowFidelityPDE(const std::tr1::shared_ptr<trrom::Vector<double> > & high_fidelity_rhs_,
                                                ...
                                                std::tr1::shared_ptr<trrom::Vector<double> > & high_fidelity_solution_)
{
    ...
    m_LeftHandSideActiveIndices->gemv(true, 1., *high_fidelity_lhs_, 0., *m_LeftHandSideVectorTimesIndexMatrix);
    ...
    for(int index = 0; index < num_deim_coefficients; ++index)
    {
        ...
        m_ReducedStateLeftHandSide->add(this_coefficient, *m_ReducedStateLeftHandSideEnsemble[index]);
    }
                                                       ...
                                                       std::tr1::shared_ptr<trrom::Vector<double> > & high_fidelity_solution_)
{
    ...
    {
        double this_coefficient = (*m_LeftHandSideDeimCoefficients)[index];
        m_ReducedDualLeftHandSide->add(this_coefficient, *m_ReducedDualLeftHandSideEnsemble[index]);
    }
...
{
    /// Discrete Empirical Interpolation Method (DEIM) routine
    std::tr1::shared_ptr<trrom::Vector<double> > residual = m_Data->createLeftHandSideSnapshotCopy();
    residual->copy(m_LeftHandSideBasis->vector(0));
    residual->modulus();
    int max_index = 0;
    residual->max(max_index);

    m_LeftHandSideActiveIndices->fill(0);
    m_LeftHandSideActiveIndices->operator()(max_index, 0) = 1;

    int num_basis_vectors = m_LeftHandSideBasis->numCols();
    int num_degress_of_freedom = m_LeftHandSideBasis->numRows();
    ...
    for(int basis_vector_index = 1; basis_vector_index < num_basis_vectors; ++basis_vector_index)
    {
        ...
        residual->axpy(1., m_LeftHandSideBasis->vector(basis_vector_index));
        // Compute DEIM max_index = \max(\lvert \vec{res} \rvert) and set P(max_index,basis_vector) = 1
        residual->modulus();
        residual->max(max_index);
        m_LeftHandSideActiveIndices->operator()(max_index, basis_vector_index) = 1;
    }
}

const std::tr1::shared_ptr<trrom::Matrix<double> > & ReducedBasisInterface::getLeftHandSideActiveIndices() const
{
