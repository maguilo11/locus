                            ...
                            std::tr1::shared_ptr<trrom::Matrix<double> > & right_singular_vectors_)
{
    ...
    std::tr1::shared_ptr<trrom::Matrix<double> > M = data_set_->create(num_rows, num_columns);
    ...
    int num_dofs = data_set_->getNumRows();
    std::tr1::shared_ptr<trrom::Matrix<double> > P_bar = data_set_->create(num_dofs, num_columns);
    ...
    std::tr1::shared_ptr<trrom::Matrix<double> > Q = data_set_->create(num_dofs, num_columns);
    std::tr1::shared_ptr<trrom::Matrix<double> > R = data_set_->create(num_columns, num_columns);
    m_OrthoFactorization->factorize(*P_bar, *Q, *R);

    // Form matrix \mathbf{K}
    std::tr1::shared_ptr<trrom::Matrix<double> > K;
    m_MatrixFactory->buildMatrixK(singular_values_, M, R, K);
    ...
    m_SpectralMethod->solve(K,
                            reduced_singular_values,
                            reduced_left_singular_vectors,
                            reduced_right_singular_vectors);
    ...
    m_MatrixFactory->buildMatrixUbar(left_singular_vectors_, Q, reduced_left_singular_vectors, Ubar);
    ...
    m_MatrixFactory->buildMatrixVbar(right_singular_vectors_, reduced_right_singular_vectors, Vbar);
