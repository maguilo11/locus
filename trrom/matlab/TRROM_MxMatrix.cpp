...
{
    assert(input_.getNumRows() == this->getNumRows());
    assert(input_.getNumCols() == this->getNumCols());
    ...
    for(int index = 0; index < my_num_elements; ++index)
    {
        my_data[index] = beta_ * my_data[index] + (alpha_ * input_data[index]);
    }
}
...
void MxMatrix::replaceGlobalValue(const int & global_row_index_,
                                  ...
                                  const double & value_)
{
    assert(global_row_index_ < this->getNumRows());
    assert(global_column_index_ < this->getNumCols());
...
{
    assert(my_row_index_ < this->getNumRows());
    assert(my_column_index_ < this->getNumCols());
...
{
    assert(static_cast<int>(mxGetM(input_)) == this->getNumRows());
    assert(static_cast<int>(mxGetN(input_)) == this->getNumCols());

    double* my_data = this->data();
    double* input_data = mxGetPr(input_);
    const size_t num_elements = mxGetNumberOfElements(input_);
...
const std::tr1::shared_ptr<trrom::Vector<double> > & MxMatrix::vector(int index_) const
{
    double* my_data = mxGetPr(m_Data);
    int my_num_rows = this->getNumRows();
    for(int index = 0; index < my_num_rows; ++index)
    {
        int my_data_index = index_ * my_num_rows + index;
        (*m_Vector)[index] = my_data[my_data_index];
    }
    return (m_Vector);
}

void MxMatrix::insert(const trrom::Vector<double> & input_, int index_)
{
    assert(input_.size() == this->getNumRows());
    if(index_ > 0)
    {
        ...
        double* my_data = mxGetPr(m_Data);
        for(int input_data_index = 0; input_data_index < input_length; ++input_data_index)
        {
            int my_data_index = index_ * input_length + input_data_index;
            my_data[my_data_index] = input_[input_data_index];
        }
    }
    else
    {
        int input_length = input_.size();
        double* my_data = mxGetPr(m_Data);
        int last_column_index = this->getNumCols() - 1;
        for(int input_data_index = 0; input_data_index < input_length; ++input_data_index)
        {
            int my_data_index = last_column_index * input_length + index_;
            my_data[my_data_index] = input_[input_data_index];
        }
    }
}
