                      ...
                      std::tr1::shared_ptr<trrom::Matrix<double> > & output_)
{
    ...
    error << "\nERROR IN: " << __FILE__ << ", LINE: " << __LINE__ << " -> RESHAPE FUNCTION IS NOT DEFINED.\n";
    std::perror(error.str().c_str());
}

void Factory::buildLocalVector(const int & length_, std::tr1::shared_ptr<trrom::Vector<double> > & output_)
{
