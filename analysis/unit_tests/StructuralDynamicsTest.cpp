...
void solve(const int& aBlockSize,
           const std::string& aConfigString,
           const Plato::CrsMatrixType& aMatrix,
           const Plato::ScalarVector& aRHS,
           Plato::ScalarVector aSolution)
{
    AMGX_SAFE_CALL(AMGX_initialize());
    AMGX_SAFE_CALL(AMGX_initialize_plugins());
    AMGX_SAFE_CALL(AMGX_install_signal_handler());

    AMGX_config_handle tConfigurationHandle;
    AMGX_SAFE_CALL(AMGX_config_create_from_file(&tConfigurationHandle, aConfigString.c_str()));

    int tDevices[1];
    int tNumDevices = 1;
    cudaGetDevice(&tDevices[0]); // Specify the current device, which is not always zero.
    MPI_Comm tComm = MPI_COMM_SELF;
    AMGX_resources_handle tRsrc = nullptr;
    AMGX_SAFE_CALL(AMGX_resources_create(&tRsrc, tConfigurationHandle, &tComm, tNumDevices, tDevices));

    const void *tDiagData = nullptr;
    const void *tData = aMatrix.entries().data();
    const int *tRowPtrs = aMatrix.rowMap().data();
    const int *tColumnIndices = aMatrix.columnIndices().data();

    AMGX_matrix_handle tA;
    AMGX_Mode tMode = AMGX_mode_dDDI; // d=device memory, D=double matrix type, D=double vector type, I=32-bit integer type.
    AMGX_SAFE_CALL(AMGX_matrix_create(&tA, tRsrc, tMode));
    const Plato::OrdinalType tNumDofs = aSolution.size();
    const Plato::OrdinalType tNNZ = aMatrix.columnIndices().size();
    AMGX_SAFE_CALL(AMGX_matrix_upload_all(tA, tNumDofs/aBlockSize, tNNZ, aBlockSize, aBlockSize, tRowPtrs, tColumnIndices, tData, tDiagData));

    AMGX_vector_handle tRHS;
    AMGX_SAFE_CALL(AMGX_vector_create(&tRHS, tRsrc, tMode));
    AMGX_SAFE_CALL(AMGX_vector_upload(tRHS, aRHS.size()/aBlockSize, aBlockSize, aRHS.data()));

    AMGX_vector_handle tLHS;
    AMGX_SAFE_CALL(AMGX_vector_create(&tLHS, tRsrc, tMode));
    AMGX_SAFE_CALL(AMGX_vector_upload(tLHS, aSolution.size()/aBlockSize, aBlockSize, aSolution.data()));

    // create eigen solver
    AMGX_eigensolver_handle tEigensolver;
    AMGX_SAFE_CALL(AMGX_eigensolver_create(&tEigensolver, tRsrc, tMode, tConfigurationHandle));
    // setup eigen solver
    AMGX_SAFE_CALL(AMGX_eigensolver_setup(tEigensolver, tA));
    // set solution's initial guess 
    AMGX_SAFE_CALL(AMGX_vector_set_random(tLHS, aSolution.size()));
    // PageRank setup
    AMGX_SAFE_CALL(AMGX_eigensolver_pagerank_setup(tEigensolver, tRHS));
    // solve eigen problem
    AMGX_SAFE_CALL(AMGX_eigensolver_solve(tEigensolver, tLHS));
    AMGX_SAFE_CALL(AMGX_vector_download(tLHS, aSolution.data()));

    // destroy memory
    AMGX_eigensolver_destroy(tEigensolver);
    AMGX_matrix_destroy(tA);
    AMGX_vector_destroy(tRHS);
    AMGX_vector_destroy(tLHS);
    AMGX_resources_destroy(tRsrc);
    AMGX_SAFE_CALL(AMGX_config_destroy(tConfigurationHandle));
...
/*
TEUCHOS_UNIT_TEST(PlatoAlexaUnitTests, SimpleEigen_PowerIteration)
{
    // SET MATERIAL PROPERTIES
    Plato::Scalar tMass = 1;
    Plato::Scalar tLength = 1;
    Plato::Scalar tInertia = 1;
    Plato::Scalar tElasticModulus = 1;

    // SET STIFFNESS MATRIX ENTRIES
    Plato::OrdinalType tNNZ = 14;
    Plato::ScalarVector tStiffEntries("StiffNonZeroValues", tNNZ);
    auto tHostEntries = Kokkos::create_mirror(tStiffEntries);
    tHostEntries(0) = 156.; tHostEntries(1) = 54.; tHostEntries(2) = -11*tLength; tHostEntries(3) = 6.5*tLength; // first row
    tHostEntries(4) = 54.; tHostEntries(5) = 312.; tHostEntries(6) = -6.5*tLength; // second row
    tHostEntries(7) = -11.*tLength; tHostEntries(8) = -6.5*tLength; tHostEntries(9) =tLength*tLength; tHostEntries(10) = -0.75*tLength*tLength; // third row
    tHostEntries(11) = 6.5*tLength; tHostEntries(12) = -0.75*tLength*tLength; tHostEntries(13) = 2.*tLength*tLength; // fouth row
    Kokkos::deep_copy(tStiffEntries, tHostEntries);
    Plato::Scalar tStiffConst = tMass*tLength / static_cast<Plato::Scalar>(840);
    Plato::scale(tStiffConst, tStiffEntries);

    // SET MASS MATRIX ENTRIES
    Plato::ScalarVector tMassEntries("MassNonZeroValues", tNNZ);
    tHostEntries = Kokkos::create_mirror(tMassEntries);
    tHostEntries(0) = 12.; tHostEntries(1) = -12.; tHostEntries(2) = -3.*tLength; tHostEntries(3) = -3.*tLength; // first row
    tHostEntries(4) = -12.; tHostEntries(5) = 24; tHostEntries(6) = 3.*tLength; // second row
    tHostEntries(7) = -3.*tLength; tHostEntries(8) = 3.*tLength; tHostEntries(9) =tLength*tLength; tHostEntries(10) = 0.5*tLength*tLength; // third row
    tHostEntries(11) = -3.*tLength; tHostEntries(12) = 0.5*tLength*tLength; tHostEntries(13) = 2*tLength*tLength; // fouth row
    Kokkos::deep_copy(tMassEntries, tHostEntries);
    Plato::Scalar tMassConst = (static_cast<Plato::Scalar>(8)*tElasticModulus*tInertia) / (tLength*tLength*tLength);
    Plato::scale(tMassConst, tMassEntries);

    // ADD STIFFNESS AND MASS MATRIX ENTRIES
    Plato::ScalarVector tEntries("NonZeroValues", tNNZ);
    Plato::axpy(static_cast<Plato::Scalar>(1), tStiffEntries, tEntries);
    Plato::axpy(static_cast<Plato::Scalar>(-1),tMassEntries, tEntries);

    // SET COLUMN INDICES
    Plato::LocalOrdinalVector tColumnIndices("NonZeroValues-ColumnIndices", tNNZ);
    auto tHostColumnIndices = Kokkos::create_mirror(tColumnIndices);
    tHostColumnIndices(0) = 0; tHostColumnIndices(1) = 1; tHostColumnIndices(2) = 2; tHostColumnIndices(3) = 3; // first row
    tHostColumnIndices(4) = 0; tHostColumnIndices(5) = 1; tHostColumnIndices(6) = 2; // second row
    tHostColumnIndices(7) = 0; tHostColumnIndices(8) = 1; tHostColumnIndices(9) = 2; tHostColumnIndices(10) = 3; // third row
    tHostColumnIndices(11) = 0; tHostColumnIndices(12) = 2; tHostColumnIndices(13) = 3; // second row
    Kokkos::deep_copy(tColumnIndices, tHostColumnIndices);

    // SET NUMBER OF NONZEROS ROW MAP
    Plato::OrdinalType tNumRows = 4;
    Plato::LocalOrdinalVector tRowMap("CummulativeNumNonZerosPerRow", tNumRows + 1);
    auto tHostRowMap = Kokkos::create_mirror(tRowMap);
    tHostRowMap(0) = 0; tHostRowMap(1) = 4; tHostRowMap(2) = 7; tHostRowMap(3) = 11; tHostRowMap(4) = 14;
    Kokkos::deep_copy(tRowMap, tHostRowMap);

    // SET CRS MATRIX
    Plato::CrsMatrixType tMatrix(tRowMap, tColumnIndices, tEntries);

    // SET EIGEN PROBLEM INPUTS
    Plato::OrdinalType tBlockSize = 1;
    Plato::ScalarVector tSolution("solution vector", tNumRows);
    Plato::ScalarVector tRHS("right-hand-side vector", tNumRows);

    char tTemp[FILENAME_MAX];
    auto tFilePath = getcwd(tTemp, FILENAME_MAX) ? std::string( tTemp ) : std::string("");
    assert(tFilePath.empty() == false);
    tFilePath.append(std::string("/unit_tests/input_files/eigen_power_iteration"));

    Plato::eigen::solve(tBlockSize, tFilePath, tMatrix, tRHS, tSolution);

    // TEST EIGENVALUE
    //Plato::Scalar tGoldScalar = -256.209;
    // TEST EIGENVECTOR
    Plato::Scalar tTolerance = 1e-3;
    std::vector<Plato::Scalar> tGoldEigenVec = { 0.529017253126607, -0.836326944176503, -0.132904254772492, -0.055085818521120 };
    auto tHostEigenVec = Kokkos::create_mirror(tSolution);
    Kokkos::deep_copy(tHostEigenVec, tSolution);
    for(size_t tIndex = 0; tIndex < tGoldEigenVec.size(); tIndex++)
    {
        TEST_FLOATING_EQUALITY(tHostEigenVec(tIndex), tGoldEigenVec[tIndex], tTolerance);
    }
}
*/
