...
TEST(DOTk_GmresTestMatrix, apply)
{
    ...
    std::shared_ptr<dotk::Vector<Real> > gold = primal->clone();
...
TEST(DOTk_PrecGMRES, gmres)
{
    ...
    std::shared_ptr<dotk::Vector<Real> > control_gold = primal->control()->clone();
    ...
    std::shared_ptr<dotk::Vector<Real> > dual_gold = primal->dual()->clone();
