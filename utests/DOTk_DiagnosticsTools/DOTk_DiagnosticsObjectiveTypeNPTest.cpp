...
TEST(DOTk_NLP, variables2)
{
    ...
    dotk::serial::vector<Real> dual(num_dual, 2.);
    dotk::serial::vector<Real> state(num_states, 4.);
    dotk::serial::vector<Real> control(num_controls, 1.);
    dotk::nlp::variables vars(state, control, dual);

    dotk::serial::vector<Real> dual_gold(num_dual, 2.);
    dotk::serial::vector<Real> state_gold(num_states, 4.);
    dotk::serial::vector<Real> control_gold(num_controls, 1.);
...
TEST(DOTk_NLP, variables1)
{
    ...
    dotk::serial::vector<Real> dual_gold(num_states, 0.);
    dotk::serial::vector<Real> state_gold(num_states, 4.);
    dotk::serial::vector<Real> control_gold(num_controls, 1.);
...
TEST(DOTk_NLP, resetField)
{
    ...
    dotk::serial::vector<Real> original_state(num_states, 2.);
    dotk::serial::vector<Real> original_control(num_controls, 3.);
...
TEST(DOTk_NLP, perturbField)
{
    ...
    dotk::serial::vector<Real> state_perturbation_vector(num_states, 4.);
    dotk::serial::vector<Real> control_perturbation_vector(num_controls, 1.);

    Real epsilon = 1e-3;

    // TEST 1: DERIVATIVE W.R.T. CONTROL
    dotk::serial::vector<Real> control_gold(num_controls, 1.001);
    ...
    // TEST 2: DERIVATIVE W.R.T. STATE
    vars.mControl->copy(control);
    dotk::serial::vector<Real> state_gold(num_states, 4.004);
...
TEST(DOTk_DiagnosticsObjectiveTypeNP, checkPartialDerivativeState)
{
    ...
    std::tr1::shared_ptr<dotk::DOTk_NocedalAndWrightObjectiveNLP> objective(new dotk::DOTk_NocedalAndWrightObjectiveNLP());
    ...
    msg.str("");
    num_states = 0;
    dotk::serial::vector<Real> empty_state_data(num_states, 1.);
