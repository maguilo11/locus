% function [cellNodes, iIdx, jIdx, iVecIdx] = generateIntrepidMesh(mesh, ...
%    nVert, numCells)
%
...
iIdx = double(reshape(iIdx_tmp', 1, numel(iIdx_tmp)));
jIdx = double(reshape(jIdx_tmp', 1, numel(jIdx_tmp), 1));
clear iIdx_tmp;
clear jIdx_tmp;

% get rhs array index
iVecIdx = double(reshape(mesh.t',1,numel(mesh.t)));
