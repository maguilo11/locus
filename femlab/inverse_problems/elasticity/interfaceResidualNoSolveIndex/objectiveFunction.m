...
IndexedExpState = GLB_INVP.IndexMat*GLB_INVP.exp_state;
data_at_dof = IndexedExpState(GLB_INVP.mesh.d');
...
residual_misfit = [ (alpha * GLB_INVP.theta * ((GLB_INVP.state'*dK_dev) - ...
                      (IndexedExpState'*exp_dK_dev)))'; ...
                    (alpha * GLB_INVP.theta * ((GLB_INVP.state'*dK_vol) - ...
                      (IndexedExpState'*exp_dK_vol)))'];
...
%%%%%%%%%%% evaluate strain field at the cubature points
IndexedExpState = GLB_INVP.IndexMat*GLB_INVP.exp_state;
exp_state_at_dof = IndexedExpState(GLB_INVP.mesh.d');
exp_strain_at_cub_points = zeros(3, numCubPoints, numCells);
intrepid_evaluate(exp_strain_at_cub_points, exp_state_at_dof, GLB_INVP.Bmat);
...
upsilon = du'*residual;
alpha = (GLB_INVP.state'*residual - IndexedExpState'*residual);
gamma = (GLB_INVP.state'*(dKdev*IndexedExpState) - IndexedExpState'*(dKdev*IndexedExpState)) + ...
    (GLB_INVP.state'*(dKvol*IndexedExpState) - IndexedExpState'*(dKvol*IndexedExpState));
...
Lzu_times_du = [upsilon * ((GLB_INVP.state'*exp_dK_dev) - (IndexedExpState'*exp_dK_dev))'; ...
                upsilon * ((GLB_INVP.state'*exp_dK_vol) - (IndexedExpState'*exp_dK_vol))' ];
Lzz_times_dz = [gamma * ((GLB_INVP.state'*exp_dK_dev) - (IndexedExpState'*exp_dK_dev))'; ...
                gamma * ((GLB_INVP.state'*exp_dK_vol) - (IndexedExpState'*exp_dK_vol))' ];
...
%%%%%%%%%%% compute potential contribution
beta = (state'*(dK_dev*IndexedExpState) - IndexedExpState'*(dK_dev*IndexedExpState)) + ...
    (state'*(dK_vol*IndexedExpState) - IndexedExpState'*(dK_vol*IndexedExpState));
potential = [beta * GLB_INVP.theta * ((state'*exp_dK_dev) - (IndexedExpState'*exp_dK_dev))'; ...
             beta * GLB_INVP.theta * ((state'*exp_dK_vol) - (IndexedExpState'*exp_dK_vol))' ];
