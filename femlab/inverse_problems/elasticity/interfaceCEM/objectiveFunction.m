...
residual = K*GLB_INVP.exp_state;
potential = 0.5 * (state'*residual - GLB_INVP.exp_state'*residual) * ...
...
residual_misfit = [ (alpha*((state'*exp_dK_dev) - ...
                      (GLB_INVP.exp_state'*exp_dK_dev)))'; ...
                    (alpha*((state'*exp_dK_vol) - ...
...
%%%% compute output 
output = alpha*(dK*GLB_INVP.exp_state) + (beta*residual);
...
%%%% compute output
output = [ (alpha*(dstate'*exp_dK_dev) + (beta*(state'*exp_dK_dev - GLB_INVP.exp_state'*exp_dK_dev)))'; ...
           (alpha*(dstate'*exp_dK_vol) + (beta*(state'*exp_dK_vol - GLB_INVP.exp_state'*exp_dK_vol)))' ];
...
beta = (state'*(dK_dev*GLB_INVP.exp_state) - GLB_INVP.exp_state'*(dK_dev*GLB_INVP.exp_state)) + ...
    (state'*(dK_vol*GLB_INVP.exp_state) - GLB_INVP.exp_state'*(dK_vol*GLB_INVP.exp_state));
potential = [beta * ((state'*exp_dK_dev) - (GLB_INVP.exp_state'*exp_dK_dev))'; ...
             beta * ((state'*exp_dK_vol) - (GLB_INVP.exp_state'*exp_dK_vol))' ];
