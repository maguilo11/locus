...
%addpath /Users/miguelaguilo/Research/femlab/inverse/elasticity/interfaceL2/
addpath /Users/miguelaguilo/Research/femlab/inverse/elasticity/interfaceResidualNoSolveIndex/

global GLB_INVP;
read = true;

% Introduce the problem
fprintf('\n*** Inverse Elasticity - Potential Error Minimization ***\n');
...
Domain.nx = 150;    % num intervals in x-dir
Domain.ny = 150;    % num intervals in y-dir
% choice of synthetic data
% 'egg' = egg; 'circle' = 1D/2D/3D circle; 'diamond' = 1D/2D/3D circle in diamond
target_control = 'circle';
...
rhs_fn=@(struc)generateTraction(struc);
...
Inputs.NumberControls = 2*GLB_INVP.nVertGrid;
...
GLB_INVP.IndexMat(1:2:Inputs.NumberStates,1:2:Inputs.NumberStates) = 0;

% Compute normalization constants
rhs = rhs_fn(GLB_INVP);
GLB_INVP.pdata = target_control;
bulk = Inputs.InitialControl(1+GLB_INVP.nVertGrid:end,1);
shear = Inputs.InitialControl(1:GLB_INVP.nVertGrid,1);
[state,K] = solvePDE(GLB_INVP,shear,bulk);
IndexedExpState = GLB_INVP.IndexMat*GLB_INVP.exp_state;
residual = K*IndexedExpState - GLB_INVP.force;
potential_energy = state'*residual - IndexedExpState'*residual;
...
fprintf(1,' \n');
fprintf(1, ' Solve Inverse Elastostatics Problem \n');
