...
misfit = 0.5 * GLB_INVP.gamma * (data_misfit' * (GLB_INVP.M * data_misfit ));

% test
% test1n = 0.5 * GLB_INVP.theta * ...
%     (GLB_INVP.exp_state'*(K*state) - GLB_INVP.exp_state'*(K*GLB_INVP.exp_state)) * ...
%     (GLB_INVP.exp_state'*(K*state) - GLB_INVP.exp_state'*(K*GLB_INVP.exp_state));
test1 = 0.5 * ...
    (GLB_INVP.exp_state'*(K*state) - GLB_INVP.exp_state'*(K*GLB_INVP.exp_state)) * ...
    (GLB_INVP.exp_state'*(K*state) - GLB_INVP.exp_state'*(K*GLB_INVP.exp_state));
% test2n = 0.5 * GLB_INVP.theta * ...
%     (GLB_INVP.exp_state'*(K*state) - state'*(K*state)) * ...
%     (GLB_INVP.exp_state'*(K*state) - state'*(K*state));
% test2 = 0.5 * ...
%     (GLB_INVP.exp_state'*(K*state) - state'*(K*state)) * ...
%     (GLB_INVP.exp_state'*(K*state) - state'*(K*state));
% test3n = 0.5 * GLB_INVP.theta * ...
%     ((state'*(K*GLB_INVP.exp_state - GLB_INVP.force)) - (GLB_INVP.exp_state'*(K*GLB_INVP.exp_state - GLB_INVP.force))) * ...
%     ((state'*(K*GLB_INVP.exp_state - GLB_INVP.force)) - (GLB_INVP.exp_state'*(K*GLB_INVP.exp_state - GLB_INVP.force)));
test3 = 0.5 * ...
    ((state'*(K*GLB_INVP.exp_state - GLB_INVP.force)) - (GLB_INVP.exp_state'*(K*GLB_INVP.exp_state - GLB_INVP.force))) * ...
    ((state'*(K*GLB_INVP.exp_state - GLB_INVP.force)) - (GLB_INVP.exp_state'*(K*GLB_INVP.exp_state - GLB_INVP.force)));
% test4 = 0.5*(state'*(K*GLB_INVP.exp_state - GLB_INVP.force))*(state'*(K*GLB_INVP.exp_state - GLB_INVP.force));

%%%
output = potential + misfit + reg;
...
potential = 2*alpha*GLB_INVP.theta*K*state;

%% data misfit
data_misfit = state - GLB_INVP.exp_state;
misfit = GLB_INVP.gamma * GLB_INVP.M * data_misfit;

%% output
output = potential + misfit;
...
%%%% compute output
beta = state'*(K*dstate);
potential = 2*GLB_INVP.theta*(alpha*(K*dstate) + (2*beta*(K*state)));

%% data misfit
misfit = GLB_INVP.gamma .* (GLB_INVP.M*dstate);
