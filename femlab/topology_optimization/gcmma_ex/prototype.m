...
control_stagnation_tolerance = 1e-3;
...
while(iteration <= max_outer_itr)
    ...
    [stop,norm_residual] = stoppingCriteriaSatisfied(primal,dual,...
    ...
    if(stop == true)
        why = 'residual';
        break;
    elseif(grad_norm  < grad_tolerance)
        why = 'grad_norm';
        break;
    elseif(control_stagnation_norm < control_stagnation_tolerance)
        why = 'control_stagnation_norm';
        break;
    elseif(max(feasibility_measure) < feasibility_tolerance)
        why = 'feasibility_measure';
        break;
    elseif(norm_residual < residual_tolerance)
        why = 'norm_residual';
                break;
    end
    ...
    [function_values,feasibility_measure,state,primal,dual,rho,active_set] = ...
...
for index=1:number_inequalities
    function_values(1+index) = -inequality.value(index) + ...
        inequality.evaluate(state,control,index);
...
function [new_function_values,feasibility_measure,state,primal,dual,rho,active_set] = ...
...
while(iteration <= max_inner_itr)
    ...
    delta_primal = norm(old_primal-primal);
