...
primal(1:number_controls) = ((dataMng.lower_asymptote.*(a_term.^0.5)) + ...
    (dataMng.upper_asymptote.*(b_term.^0.5))) ./ ...
    ((a_term.^0.5)+(b_term.^0.5));
% Update auxiliary variables Y_i
primal(number_controls+1:number_controls+number_inequalities)=...
    (trial_dual - sum(dataMng.c_coeff)) ./ sum(dataMng.d_coeff);
...
[primal] = projectPrimal(primal,dataMng);
end

% project primal
function [primal] = projectPrimal(primal,dataMng)
...
% C++ implementation requires loov over controls
