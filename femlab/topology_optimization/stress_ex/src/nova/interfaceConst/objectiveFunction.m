...
output = (GLB_INVP.constant * stress_measure) + perimeter_control;
...
%%%%%%%%%%% compute cell objective measure
control_at_dof = control( GLB_INVP.mesh.t');
elem_von_mises = zeros(GLB_INVP.numCells,1);
elem_stress_measure = zeros(GLB_INVP.numCells,1);
...
for cell=1:GLB_INVP.numCells
    ...
    elem_von_mises(cell) = sqrt(von_mises_stress_square);
    elem_stress_ratio = elem_von_mises(cell) / GLB_INVP.StressNormFactor(cell);
    ...
    elem_coeff = (0.5/(elem_von_mises(cell) * GLB_INVP.StressNormFactor(cell))) * elem_coeff;
...
if(GLB_INVP.iteration > 1)
    %%%%%%%%%%% Update constant
    constant = (GLB_INVP.max_stress_measure/GLB_INVP.stress_measure);
    if(GLB_INVP.constant < constant)
        alpha = 0.95;
        constant = GLB_INVP.alpha * (GLB_INVP.max_stress_measure/GLB_INVP.stress_measure) + ...
            (1 - alpha)*GLB_INVP.constant;
    end
    GLB_INVP.constant = constant;
end
GLB_INVP.max_stress_measure = max(elem_von_mises);
GLB_INVP.stress_measure = (sum(elem_stress_measure))^(1/GLB_INVP.PowerKS);
...
output = ((1/GLB_INVP.PowerKS) * sum(elem_stress_measure)^((1/GLB_INVP.PowerKS) - 1)) .* (Mass2 * one);
output = GLB_INVP.constant .* output;
...
for cell=1:GLB_INVP.numCells
    %%%% Primary Coefficients
    cell_mass_volume = sum( GLB_INVP.CellMassMatrices(:,:,cell)*control_at_dof(:,cell) );
    ...
    cell_sensitivities(:,cell) = (GLB_INVP.ElemVolume(cell) * GLB_INVP.PowerKS * ...
        stress_ratio^(GLB_INVP.PowerKS-1)) * (0.5/(GLB_INVP.StressNormFactor(cell) * ...
        cell_von_mises_stress)) .* cell_sensitivities(:,cell);

end
constant = ((1/GLB_INVP.PowerKS) * sum(cell_stress_measure)^((1/GLB_INVP.PowerKS) - 1));
