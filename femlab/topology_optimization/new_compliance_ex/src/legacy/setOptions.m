...
switch Inputs.SolutionType
    ...
    case 'SteihaugToint'
        Options = setSteihaugToint(Inputs, Options);
    case 'IxSteihaugToint'
        Options = setIxSteihaugToint(Inputs, Options);
...
function [Options] = setSteihaugToint(Inputs, Options)
Options = setProblem(Inputs, Options);
Options = setGeneralOptions(Options);
Options = setTrustRegionMethod(Options);
Options = setGradientComputationMethod(Options);
...
function [Options] = setIxSteihaugToint(Inputs, Options)
Options = setProblem(Inputs, Options);
Options = setGeneralOptions(Options);
Options = setTrustRegionMethod(Options);
Options = setGradientComputationMethod(Options);
...
Options.MinTrustRegionRadius = 1e-7;
Options.InitialTrustRegionRadius = 1e0;
Options.TrustRegionExpansionFactor = 2;
Options.TrustRegionContractionFactor = 0.5;
Options.MaxNumTrustRegionSubProblemItr = 5;
Options.MaxActualOverPredictedReductionRatio = 0.8;
Options.MinActualOverPredictedReductionRatio = 0.2;
end
...
%  10. LIU_STOREY
%  11. DANIELS
Options.NonlinearCgMethod = 'LIU_STOREY';
if (strcmp(Options.NonlinearCgMethod,'LIU_STOREY'))
    Options = setNumericalDifferentiationMethod(Options);
end
end
