...
state = zeros(spaceDim*nVertGrid,numLoadCases);
...
for index=1:numLoadCases
    ...
    if( ~isempty(state(:,index)) )
        state(unique(GLB_INVP.dirichlet),index) = ...
            GLB_INVP.u_dirichlet( unique(GLB_INVP.dirichlet) );
        force = GLB_INVP.force(:,index) - K * state(:,index);
    end
    %%%%%%%%%%% Solve system of equations
    if(State.highFidelity == true)
        %%%%%%%%%%% High fidelity solve
        state(GLB_INVP.FreeDof,index) = ...
...
%             K(GLB_INVP.FreeDof,GLB_INVP.FreeDof) \ force(GLB_INVP.FreeDof);
        %%%%%%%%%%% Low fidelity solve
        Kr = State.basis' * K * State.basis;
        Fr = State.basis' * force;
        State.basisCoeff = Kr \ Fr;
        state(:,index) = State.basis*State.basisCoeff;
    end
end
State.current = state;
...
function [output] = ...
    secondDerivativeWrtControlControl(State,Control,Dual,dcontrol)

global GLB_INVP;

%%%%%%%%%%% Set neccesary data for calculations
...
numLoadCases = size(GLB_INVP.force,2);
output = zeros(size(Control.current,1),1);
for index=1:numLoadCases
    %%%%%%%%%%% Compute Penalty Parameters
    state = State.current(:,index);
    state_at_dof = state(GLB_INVP.mesh.d');
    dual_at_dof = -GLB_INVP.theta(index)*state_at_dof;
    for cell=1:GLB_INVP.numCells
        penalty = ...
            sum(GLB_INVP.CellMassMatrices(:,:,cell) * control_at_dof(:,cell)) ...
            / GLB_INVP.ElemVolume(cell);
