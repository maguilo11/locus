...
% Grab nodes.
coordx = exomesh(str_coordx);
coordy = exomesh(str_coordy);
mesh.p = zeros(size(coordx,1),2);
mesh.p(:,1) = coordx;
mesh.p(:,2) = coordy;
...
mesh.numSideSet = nss-1;

% Grab nodesets and compute global nodes associated with nodesets.
nns = 1;
nodeset_key = strcat(str_node_ns, '1');
while exomesh.isKey(nodeset_key)
    mesh.nodeset{nns} = double(exomesh(nodeset_key));
    
    nns = nns+1;
    nodeset_key = strcat(nodeset_key, num2str(nss));
end
mesh.numNodeSet = nns-1;
